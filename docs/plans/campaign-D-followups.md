...
steps: (a) PERF GATE — fast-iterate pilot first: `run_step.sh dp-pilot -- scripts/run_in_rocm.sh python3 scripts/evals/dp/dp_leakage_sweep.py --layers 12 --epsilons inf,256 --out refine-logs/dp-decoder-grid/pilot.json`, confirm finite bits + pool size + sigma_convention in the JSON and estimate wall-time; (b) full run `TALENS_SURFACE=dp-decoder-grid run_step.sh dp-sweep -- scripts/run_in_rocm.sh python3 scripts/evals/dp/dp_leakage_sweep.py --layers 0,5,12,20 --epsilons inf,1024,512,256 --attacks ridge,decoder --probes club,vcap --out refine-logs/dp-decoder-grid/dp_leakage_sweep.json` (ONE GPU process, serial); (c) /result-to-claim + /experiment-audit (probe≠attack: ridge/decoder are ATTACKS, CLUB/V_cap independent probes — the eval already separates them); (d) /auto-review-loop.
...
objective: re-run `scripts/evals/dp/dp_leakage_sweep.py` on the FULL 512-prompt corpus (≈3× the 160-prompt data — the binding constraint per the decoder research) with the R2 decoder now in the eval (GELU, gated zero-init skip, FROZEN ridge-warm-started linear path, narrow h≈384, early-stop on a disjoint val split), grid L0/5/12/20/25 × ε, then refresh FIG.02 + the table on resid-dp-attacks.html with the new numbers.
pointers — consume: the updated `scripts/evals/dp/dp_leakage_sweep.py` (R2 decoder + warm-start, implemented 2026-06-25). produce: `refine-logs/dp-decoder-r2/dp_leakage_sweep.json` and the refreshed FIG.02 data + collapsed table on `docs/html/resid-dp-attacks.html`. do-not-redo: do NOT widen h (narrow is correct); do NOT re-derive the decoder (it's done); one GPU process via run_step.sh; perf-gate (fast-iterate pilot) FIRST; this is the full version of the data already partially shown (160 prompts) on the page. recipe-fit: `full` GPU mechanics + report refresh; no new claim (the affine-saturation claim already exists).
...
impl-context (2026-06-25, from the BeamClean design pass — READ BEFORE BUILDING):
  - Formulation = noisy-channel sequence decode: argmax_{v₁..v_T} Σ_t [ log p_emission(o_t | v_t) + λ·log p_LM(v_t | v_<t) ], beam width B, left-to-right. λ→0 must reproduce ridge (sanity); λ>0 should help, more as ε shrinks.
  - EMISSION: at L0 (noised embedding) it is EXACT Gaussian — log p(o_t|v) ∝ −‖o_t − clip(E[v])‖²/2σ². At L>0 the input noise has propagated through L layers (no closed form) → reuse the COSINE logit-lens posterior as the emission (log-softmax of the cosine logits). Use the cosine lens (`logit_lens_attack` in src/talens/attacks/dp_inversion.py), NOT raw dot product — the dot-product/norm bug (init dot 0.78 vs cosine 1.00) was diagnosed + fixed 2026-06-25 (commit 7c32893).
  - PRIOR LM: gemma-2-2b itself (already loaded in the sweep; public under WEIGHTS-PUB) — read its next-token logits over the candidate pool. No extra model.
  - KEY HARNESS CHANGE: BeamClean is a SEQUENCE attack and does NOT fit dp_leakage_sweep.py (a flattened, vocab-DISJOINT, per-position bag — the prior needs CONTIGUOUS preceding tokens, which the disjoint split scatters). So an `--attacks beamclean` option in dp_leakage_sweep is NOT viable; build a SIBLING `scripts/evals/beamclean_seq_sweep.py` that: capture() already returns per-prompt ordered residual sequences → beam-decode each whole prompt → metric = SEQUENCE token-F1 / exact-match vs the true prompt, swept over ε. This is a NEW surface (reconstruct the actual input text), reported separately from the per-position page.
  - MANDATORY control (integrity-critical): a PRIOR-ONLY baseline (λ→∞ / emission off = pure LM generation). A strong LM recovers plausible text from almost no signal; without this control the "recovery" can be LM hallucination, not leakage. Report BeamClean ABOVE the prior-only floor.
  - WHY NOW: the per-vector axis is settled SATURATED, triangulated 2026-06-25 — r3-fixed cosine-regression gated decoder == ridge at all cells; ce-lens cosine-CE declens == lens, and CE-retraining the affine even LOSES to closed-form ridge (open-set overfit). So a deeper per-vector net cannot beat ridge; BeamClean (sequence + LM prior) is the ONLY remaining axis for >ridge recovery. See research-wiki/claims/single-position-residual-linearly-saturated.md + docs/research/ce-logit-lens-attack.md.
  - COST: largest remaining build — new attack module + new SEQUENCE eval + λ/B sweep + the prior-only control; multi-hour, multi-GPU-run (LM scoring runs gemma autoregressively over beams). Reuse: capture() (sequences), logit_lens_attack (emission), gemma (prior).
acceptance: a reviewed BeamClean attack exists under src/talens/attacks/ (passed /auto-review-loop); a SEQUENCE-level DP ε-sweep (token-F1/exact-match per prompt) shows it beats both ridge AND the prior-only (λ→∞) baseline, with the gap over ridge WIDENING as ε shrinks (high noise); λ→0 reproduces ridge; results in refine-logs/dp-beamclean/; a new sequence-recovery page (or section) gains the LM-prior attack series + a claim; perf gate passed.
