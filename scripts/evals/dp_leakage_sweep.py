...
def _ridge_W(Xtr, Etr, alpha=1.0):
...
def attack_ridge(Xtr, Etr, Xte, pool_emb, pool_ids, *, alpha=1.0, **_):
    ...
    return _nearest_token(Xte @ _ridge_W(Xtr, Etr, alpha), pool_emb, pool_ids)


class _SkipDecoder(torch.nn.Module):
    ...
    def __init__(self, d_in, d_out, hidden):
        ...
        self.mlp = torch.nn.Sequential(torch.nn.Linear(d_in, hidden), torch.nn.GELU(), torch.nn.Linear(hidden, d_out))
...
def attack_decoder(Xtr, Etr, Xte, pool_emb, pool_ids, *, hidden=384, epochs=500, lr=1e-3, seed=0,
                   val_frac=0.15, patience=40, **_):
    ...
    zero MLP tail at init) trains, with weight-decay and early-stopping on a held-out split. So the
    decoder starts identical to ridge and can only add a data-supported non-linear correction —
    a clean `decoder ≥ ridge` guarantee with overfitting control. (Tuned-lens affine baseline +
    Rep2Text gated skip; see research-wiki/claims/single-position-residual-linearly-saturated.md.)
    """
    import copy
    torch.manual_seed(seed)
    net = _SkipDecoder(Xtr.shape[1], Etr.shape[1], hidden).to(DEV)
    with torch.no_grad():  # warm-start: linear path = ridge, gate=0, MLP tail=0 → starts AT ridge
        net.lin.weight.copy_(torch.from_numpy(np.ascontiguousarray(_ridge_W(Xtr, Etr).T)).to(DEV))
        net.lin.bias.zero_()
    ...
    for _ in range(epochs):
        ...
        net.eval()
        with torch.no_grad():
            pv = net(xv); pv = pv / pv.norm(dim=1, keepdim=True).clamp_min(1e-9)
